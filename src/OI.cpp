...
OI::OI() {
	oi_splitControl = Robot::splitControllers;
...
//	bButton->WhileHeld(new RaiseBoxHandler());
...
//	yButton2->WhenReleased(new LiftToHeight(39));
...
//	aButton2->WhenPressed(new LiftToHeight(1));
	...
	remapControls(Robot::splitControllers);
}
...
/**
 * Maps the control scheme to use one or two controllers.
 *
 * @param isSplit true if using two controllers.
 */

void OI::remapControls(bool isSplit) {
	oi_splitControl = isSplit;
	// reset all of the pointers.
	intakeButton.reset();
	ejectButton.reset();
	slowEjectButton.reset();
	climbButton.reset();
	shiftButton.reset();
	toggleButton.reset();
	scaleHeightButton.reset();
	switchHeightButton.reset();
	floorHeightButton.reset();

	// give each pointer a destination.
	if (isSplit) {
		intakeButton = aButton;
		ejectButton = rightBumper2;
		slowEjectButton = leftBumper2;
		climbButton = startButton;
		shiftButton = leftJoystick;
		toggleButton = bButton2;
		scaleHeightButton = yButton2;
		switchHeightButton = xButton2;
		floorHeightButton = aButton2;
	} else {
		intakeButton = aButton;
		ejectButton = rightBumper;
		slowEjectButton = leftBumper;
		climbButton = startButton;
		shiftButton = leftJoystick;
		toggleButton = bButton;
		scaleHeightButton = yButton;
//		switchHeightButton = selectButton; // unused on single controller, I left a comment to remind myself of that.
		floorHeightButton = xButton;
	}

	// use them.
	intakeButton->WhileHeld(new IntakeBox(.7));
	ejectButton->WhileHeld(new EjectBox(1.0));
	slowEjectButton->WhileHeld(new IntakeBox(.7));
	climbButton->WhileHeld(new Ascend(1));

	toggleButton->WhenPressed(new ToggleBoxPiston());
	scaleHeightButton->WhenPressed(new LiftToHeight(39));
	floorHeightButton->WhenPressed(new LiftToHeight(2));
	if (isSplit) { // not used on single controller.
		switchHeightButton->WhenPressed(new LiftToHeight(17));
	} else { // used only on single controller.
		// nothing to see here. move on.
	}
}

double OI::getDriveJoystick() {
	if(oi_splitControl) {
